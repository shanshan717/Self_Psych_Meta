...
# Import data
data_path <- "../1_Data/AnalysisData/Literature_info/Sample_info.csv"
...
country_summary <- Sample_Info %>% 
  ...
  dplyr::summarise(Total_Sample = base::sum(patients_sample, na.rm = TRUE))

# Merge map coordinates with statistical data using dplyr
world_with_data <- world %>% 
  dplyr::left_join(country_summary, by = c("region" = "country"))
...
df_stats <- Sample_Info %>% 
  ...
  dplyr::mutate(group_disorder_name = stringr::str_trim(group_disorder_name)) %>%
  dplyr::mutate(group_disorder_name = forcats::fct_infreq(group_disorder_name))

# Descriptive statistics in console
base::print(base::table(df_stats$group_disorder_name))
total_studies <- base::nrow(df_stats)
base::print(base::paste("Total number of studies analyzed:", total_studies))

# Generate bar and jitter plot
p_bar <- ggplot2::ggplot(df_stats, ggplot2::aes(x = group_disorder_name)) +
  ...
  ggplot2::geom_jitter(
    ggplot2::aes(y = patients_sample, color = Disease_criteria),
